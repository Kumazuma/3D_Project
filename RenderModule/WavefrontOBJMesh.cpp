...
WavefrontOBJMesh::WavefrontOBJMesh(RenderModule* pRenderModule, std::wstring const& path):
    RenderObject{}
{
...
auto WavefrontOBJMesh::ParseOBJFile(RenderModule* pRenderModule, std::wstring const& path, std::wstring* pOutMeterialFilePath) -> void
{
    HRESULT hr{};
...
#pragma endregion
    ...
    for (auto& shape: shapes)
    {
        ...
        WavefrontOBJSubMesh subset{ pRenderModule, m_pVertexPositions, materialName, std::move(indices) };
        m_subsets.emplace(name, std::make_shared<WavefrontOBJSubMesh>(std::move(subset)));
    }
...
WavefrontOBJMesh::WavefrontOBJMesh(WavefrontOBJMesh const& rhs):
    RenderObject{ rhs },
...
WavefrontOBJMesh::WavefrontOBJMesh(WavefrontOBJMesh&& rhs) noexcept :
    RenderObject{ rhs },
...
auto WavefrontOBJMesh::Create(RenderModule* pRenderModule, std::wstring const& path, WavefrontOBJMesh** pOut) -> HRESULT
{
    try
    {
        if (pOut == nullptr)
            throw E_POINTER;
        WavefrontOBJMesh obj{ pRenderModule, path };
        *pOut = new WavefrontOBJMesh{ obj };
    }
    catch (HRESULT hr)
    {
        return hr;
...
{
    return new WavefrontOBJMesh{ *this };
}

auto WavefrontOBJMesh::GetVertexCount() const -> u32
{
    return m_vertexCount;
}
auto WavefrontOBJMesh::GetCenter() const -> DirectX::XMFLOAT3A const&
{
    return m_center;
}

auto WavefrontOBJMesh::CanRayPicking() const -> bool 
{
    return true;
}
...
auto WavefrontOBJMesh::GetSubsets() const -> std::unordered_map<std::wstring, std::shared_ptr<WavefrontOBJSubMesh>> const&
{
    return m_subsets;
}

WavefrontOBJSubMesh::WavefrontOBJSubMesh(RenderModule* pRenderModule, std::shared_ptr<std::vector<DirectX::XMFLOAT3A> > const & vertexPositions, std::wstring const& materialName, std::vector<Triangle>&& indices):
    ...
    m_pVertexPositions{vertexPositions}
{
    ...
    hr = pDevice->CreateIndexBuffer(
        static_cast<UINT>(WavefrontOBJMesh::INDEX_SIZE * indices.size()),
        D3DUSAGE_WRITEONLY,
        WavefrontOBJMesh::INDEX_TYPE,
        D3DPOOL_MANAGED,
        &m_pIndexBuffer,
        nullptr
    );
    ...
    memcpy_s(pIndices, WavefrontOBJMesh::INDEX_SIZE * indices.size(), indices.data(), WavefrontOBJMesh::INDEX_SIZE * indices.size());
...
WavefrontOBJSubMesh::WavefrontOBJSubMesh(WavefrontOBJSubMesh&& rhs) noexcept:
...
auto WavefrontOBJSubMesh::GetIndicesRef() const -> std::vector<Triangle> const&
{
    return m_indices;
}

auto WavefrontOBJSubMesh::GetIndexBuffer() -> COMPtr<IDirect3DIndexBuffer9>
{
    return m_pIndexBuffer;
}

auto WavefrontOBJSubMesh::GetTriangleCount() const -> u32
{
    return static_cast<u32>(m_indices.size());
}

auto WavefrontOBJSubMesh::GetMaterialName() const -> std::wstring const&
{
    return m_materialName;
}

auto WavefrontOBJSubMesh::GetRadius() const -> f32
{
    return m_radius;
}

auto WavefrontOBJSubMesh::GetCenter() const -> DirectX::XMFLOAT3A const&
{
    return m_center;
}

auto __vectorcall WavefrontOBJSubMesh::RayPicking(DirectX::XMVECTOR rayAt, DirectX::XMVECTOR rayDirection, f32* pOut) -> bool
{
...
auto WavefrontOBJSubMesh::GetTriangles() const -> std::vector<Triangle> const&
{
